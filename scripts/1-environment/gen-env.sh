...
check_external_database() {
    ...
    if [ "$EXTERNAL_DB" = "True" ]; then
        ...
        read -erp "FieldTM DB Host: " FMTM_DB_HOST
        echo
        export FMTM_DB_HOST=${FMTM_DB_HOST}

        echo "Please enter the database name."
        read -erp "FieldTM DB Name: " FMTM_DB_NAME
        echo
        export FMTM_DB_NAME=${FMTM_DB_NAME}

        echo "Please enter the database user."
        read -erp "FieldTM DB User: " FMTM_DB_USER
        echo
        export FMTM_DB_USER=${FMTM_DB_USER}

        echo "Please enter the database password."
        read -erp "FieldTM DB Password: " FMTM_DB_PASSWORD
        echo
...
set_domains() {
    pretty_echo "FieldTM Domain Name"

    echo "To run FieldTM you must own a domain name."
    while true; do
        read -erp "Enter a valid domain name you wish to run FieldTM from: " fmtm_domain
    ...
    echo
    echo "Using $fmtm_domain as your main domain for FieldTM."
    echo
    ...
    echo "FieldTM will automatically generate SSL (HTTPS) certificates for your domain name."
    while true; do
...
check_change_port() {
    ...
    if [ -n "$fmtm_port" ]; then
        ...
        echo "Using port 7050 for FieldTM."
    fi
}
...
prompt_user_gen_dotenv() {
    pretty_echo "Generate dotenv config for FieldTM"
    check_existing_dotenv
    install_envsubst_if_missing
    check_if_test
