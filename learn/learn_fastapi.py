...
async def read_item(item_id: str):
    ...
    return {
        "item_id": item_id,
        "about": "Takes parameter with automatic type check",
        "item": items[item_id],
    }
...
class Lung(BaseModel):
    image: Image = Body(
        examples={
            "normal": {"url": "http://example.com/baz.jpg", "name": "The Foo live"},
            "smoker": {"url": "http://example.com/baz.jpg", "name": "The Foo dead"},
        },
    )
...
async def create_item(heart: Heart):
    if heart.broken:
        return {
            "message": "Yesterday.... All my troubles seemed so far away... Now it seems as though they're here to stay... Oh, I believe in yesterday..."
        }
    return heart
...
async def read_items(
    q: Union[str, None] = Query(default=None, min_length=1, max_length=3)
):
...
async def regex_validation(
    ...
    return {
        "message": "^: starts with the following characters, doesn't have characters before. regex: has the exact value regex. $: ends there, doesn't have any more characters after fixedquery."
    }
...
async def required_and_accepts_none(
...
async def read_something_and_know_what_it_is(
    path: int = Path(
        title="The ID of the item to get. Greater to or equal to 1. You can also use gt (greater than) or le (less than or equal)",
        ge=1,
    ),
    q: Union[str, None] = Query(
        default=None,
