...
1. By default, the docker setup includes a Central server.
2. Create a user with the email you provided in the **ODK_CENTRAL_USER** field of your `.env` file using the following command:    
  `docker compose exec central odk-cmd --email YOUREMAIL@ADDRESSHERE.com user-create`
3. When prompted to put a password, input the password you provided in the **ODK_CENTRAL_PASSWD** field of the `.env` file
4. Promote the user created to an admin using the following command:  
