...
const FormSelection: React.FC = () => {
  ...
  const totalSteps = 5;
  return (
    <CoreModules.Stack sx={{ width: '100%', display: 'flex', flexDirection: 'row' }}>
      <CoreModules.Stack sx={{ width: '50%' }}>
        <form onSubmit={handleSubmit}>
          <FormGroup>
            <CoreModules.FormControl sx={{ mb: 3, width: '50%' }} variant="filled">
              <InputLabel
                id="form-category"
                sx={{
                  '&.Mui-focused': {
            ...
            <CoreModules.Stack
              sx={{ display: 'flex', flexDirection: 'row', width: '50%', justifyContent: 'space-between' }}
            >
              {/* Previous Button  */}
