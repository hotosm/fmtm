...
const GenerateMbTiles = ({
  setToggleGenerateModal,
  toggleGenerateModal,
  projectInfo,
}) => {
  ...
  const generateProjectTilesLoading = CoreModules.useAppSelector(
    (state) => state.project.generateProjectTilesLoading
  );
  const tilesList = CoreModules.useAppSelector(
    (state) => state.project.tilesList
  );
  ...
  const downloadMbTiles = (tileId) => {
    dispatch(
      DownloadTile(
        `${environment.baseApiUrl}/projects/download_tiles/?tile_id=${tileId}`,
        projectInfo
      )
    );
  };
  const getTilesList = () => {
    dispatch(
      GetTilesList(
        `${environment.baseApiUrl}/projects/tiles_list/${decodedId}/`
      )
    );
  };
  useEffect(() => {
