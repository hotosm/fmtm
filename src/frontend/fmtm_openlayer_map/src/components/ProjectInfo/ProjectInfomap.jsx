...
function colorRange(data, noOfRange) {
  ...
  colorCodesKeys.forEach((key, index) => {
    ...
    actualCodes.push({ min: Math.round(currentValue), max: Math.round(nextValue), color: colorCodesKeys[index] });
...
const ProjectInfomap = () => {
  ...
  const federalWiseProjectCount = taskInfo?.map((task) => ({ code: task.task_id, count: task.submission_count }))
  ...
  useEffect(() => {
    ...
    const filteredSelectedTaskGeojson = {
      ...basicGeojsonTemplate, features: taskBoundaries?.features?.filter((task) => task.properties.uid === selectedTask)
    }
    ...
    dispatch(ProjectBuildingGeojsonService(`${environment.baseApiUrl}/projects/${decodedId}/features?task_id=${selectedTask}`))
