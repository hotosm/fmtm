...
def update_project_with_upload(
    ...
    if uploaded_zip.content_type not in ["application/zip"]:
        raise HTTPException(
            status_code=415, detail=f"File must be a zip. Uploaded file was {uploaded_zip.content_type}")
    ...
    with ZipFile(io.BytesIO(uploaded_zip.file.read()), 'r') as zip:
        ...
        if QR_CODES_DIR not in listed_files:
            raise HTTPException(
                status_code=400, detail=f'Zip must contain directory named {QR_CODES_DIR}')

        if TASK_GEOJSON_DIR not in listed_files:
            raise HTTPException(
                status_code=400, detail=f'Zip must contain directory named {TASK_GEOJSON_DIR}')

        outline_filename = f'{project_name_prefix}.geojson'
        if outline_filename not in listed_files:
            raise HTTPException(
                status_code=400, detail=f'Zip must contain file named "{outline_filename}" that contains a FeatureCollection outlining the project')

        task_outlines_filename = f'{project_name_prefix}_polygons.geojson'
        if task_outlines_filename not in listed_files:
            raise HTTPException(
                status_code=400, detail=f'Zip must contain file named "{task_outlines_filename}" that contains a FeatureCollection where each Feature outlines a task')
        ...
        project_tasks_feature_collection = get_json_from_zip(
            zip, task_outlines_filename, f'Could not generate FeatureCollection from {task_outlines_filename}')
        ...
        # Unexpected exception
            ...
            raise HTTPException(
                status_code=500, detail=f'{task_count} tasks were created before the following error was thrown: {e}, on feature: {feature}')
...
def get_outline_from_geojson_file_in_zip(zip, filename: str, error_detail: str, feature_index: int = 0):
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f'{error_detail} ----- Error: {e} ---- FeatureCollection: {feature_collection}')
    ...
    if db_project:
        ...
        if (db_project.project_info and len(db_project.project_info) > 0):
            default_project_info = next(
                (x for x in db_project.project_info if x.locale == db_project.default_locale), None)
