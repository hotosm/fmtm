...
def upgrade() -> None:
    ...
    sa.Column('status', sa.Enum('PENDING', 'FAILED', 'RECEIVED', 'SUCCESS', name='backgroundtaskstatus'), nullable=False),
    ...
    op.create_table('mapping_issue_categories',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    sa.Column('type', sa.Enum('FREE', 'DISCOUNTED', 'FULL_FEE', name='organisationtype'), nullable=False),
    ...
    sa.Column('role', sa.Enum('MAPPER', 'ADMIN', 'VALIDATOR', 'FIELD_ADMIN', 'ORGANIZATION_ADMIN', 'READ_ONLY', name='userrole'), nullable=False),
    sa.Column('name', sa.String(), nullable=True),
    ...
    sa.Column('mapping_level', sa.Enum('BEGINNER', 'INTERMEDIATE', 'ADVANCED', name='mappinglevel'), nullable=False),
    ...
    op.create_table('organisation_managers',
    ...
    sa.Column('task_creation_mode', sa.Enum('GRID', 'ROADS', 'UPLOAD', name='taskcreationmode'), nullable=False),
    ...
    sa.Column('status', sa.Enum('ARCHIVED', 'PUBLISHED', 'DRAFT', name='projectstatus'), nullable=False),
    ...
    sa.Column('mapper_level', sa.Enum('BEGINNER', 'INTERMEDIATE', 'ADVANCED', name='mappinglevel'), nullable=False),
    sa.Column('priority', sa.Enum('URGENT', 'HIGH', 'MEDIUM', 'LOW', name='projectpriority'), nullable=True),
    ...
    op.create_index('idx_geometry', 'projects', ['outline'], unique=False, postgresql_using='gist')
    ...
    sa.Column('visibility', sa.Enum('PUBLIC', 'PRIVATE', name='teamvisibility'), nullable=False),
    sa.ForeignKeyConstraint(['organisation_id'], ['organisations.id'], name='fk_organisations'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('task_status', sa.Enum('READY', 'LOCKED_FOR_MAPPING', 'MAPPED', 'LOCKED_FOR_VALIDATION', 'VALIDATED', 'INVALIDATED', 'BAD', 'SPLIT', 'ARCHIVED', name='taskstatus'), nullable=True),
    ...
    sa.ForeignKeyConstraint(['category_title'], ['xlsforms.title'], name='fk_xform'),
    sa.ForeignKeyConstraint(['project_id'], ['projects.id'], ),
    sa.ForeignKeyConstraint(['task_id', 'project_id'], ['tasks.id', 'tasks.project_id'], name='fk_tasks'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_features_composite', 'features', ['task_id', 'project_id'], unique=False)
    op.create_table('task_history',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('project_id', sa.Integer(), nullable=True),
    sa.Column('task_id', sa.Integer(), nullable=False),
    sa.Column('action', sa.Enum('RELEASED_FOR_MAPPING', 'LOCKED_FOR_MAPPING', 'MARKED_MAPPED', 'LOCKED_FOR_VALIDATION', 'VALIDATED', 'MARKED_INVALID', 'MARKED_BAD', 'SPLIT_NEEDED', 'RECREATED', 'COMMENT', name='taskaction'), nullable=False),
    ...
    op.create_index('idx_task_history_project_id_user_id', 'task_history', ['user_id', 'project_id'], unique=False)
    ...
    op.create_index('idx_task_validation_validator_status_composite', 'task_invalidation_history', ['invalidator_id', 'is_closed'], unique=False)
    ...
    sa.ForeignKeyConstraint(['mapping_issue_category_id'], ['mapping_issue_categories.id'], name='fk_issue_category'),
...
def downgrade() -> None:
    ...
    op.drop_index('idx_task_validation_validator_status_composite', table_name='task_invalidation_history')
