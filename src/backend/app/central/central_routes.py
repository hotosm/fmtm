...
async def get_form_lists(
    ...
    """Get a list of all XLSForms available in FieldTM.
...
async def validate_form(
    ...
    NOTE that this debug form has additional fields appended and should
        not be used for FieldTM project creation.
...
async def update_project_form(
    ...
    """Update the XForm data in ODK Central & FieldTM DB."""
...
async def upload_form_media(
    ...
    try:
        ...
        msg = (
            f"Failed to upload form media for FieldTM project ({project_id}) "
...
async def get_form_media(
    ...
    try:
        ...
        log.exception(f"Error: {e}")
        msg = (
            f"Failed to get form media for FieldTM project ({project_id}) "
            f"ODK project ({project_odk_id}) form ID ({project_xform_id})"
        )
        log.error(msg)
        raise HTTPException(
