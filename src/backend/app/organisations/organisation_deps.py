...
async def get_organisation_by_name(db: Session, org_name: str) -> DbOrganisation:
    ...
    return (
        ...
        .filter(DbOrganisation.approved==True, func.lower(DbOrganisation.name).like(func.lower(f"%{org_name}%")))
        .first()
    )
...
async def get_organisation_by_id(db: Session, org_id: int) -> DbOrganisation:
    ...
    Returns:
        DbOrganisation: organisation with the given id
    """
    return db.query(DbOrganisation).filter(DbOrganisation.id == org_id, DbOrganisation.approved==True).first()
