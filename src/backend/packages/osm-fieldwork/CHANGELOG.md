...
- Major refactoring to support conflation with either postgis or the data extract used for FieldTM
...
- use log so it works in FieldTM, get the feature version in SQL queries
...
- Convert the currt JSON format as downloaded from FieldTM
...
- Make default values work so we don't break the FieldTM frontend
