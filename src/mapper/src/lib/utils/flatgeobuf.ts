...
function extentToFgbRect(extent: bboxType | Polygon): Rect {
	...
	if (Array.isArray(extent) && extent.length === 4 && extent.every((coord) => typeof coord === 'number')) {
		return {
			minX: extent[0],
			minY: extent[1],
			maxX: extent[2],
	...
	// Check if extent is a Polygon and calculate Rect
		...
		throw new Error('The extent parameter must be a bbox array [minX, minY, maxX, maxY] or a Polygon GeoJSON object.');
	}
}
...
export function filterGeomsCentroidsWithin(
	...
	const filteredFeatures = featcol.features.filter((feature) => centroidWithin(feature.geometry, extent));
